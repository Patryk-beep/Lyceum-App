...
4. **Flag misconceptions explicitly.** For each concept in this module, pull its known misconceptions from `knowledge-map.json` (`misconceptions[]`) and call them out by name: state the misconception, why it is tempting, and the correction.
   ...
   **Then, separately, flag the live debates — a distinct move from misconceptions.** If `knowledge-map.json` has an `openQuestions` array, pull the entries whose `concept` id matches a concept in this module and present them as a clearly separate move, explicitly framed: *here is what **experts still disagree** on — the live edge of the field, **not a gap in your understanding**.* Give the competing positions fairly (each carries its source URL). Keep this firmly distinct from the misconception call-out: a misconception is a known-wrong belief to *correct*; an open question is a genuine, unsettled debate to *appreciate*. **If `openQuestions` is absent or empty, skip this move entirely — most maps have none, and that is correct; never invent open questions to fill it.**
