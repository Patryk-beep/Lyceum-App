...
4. Create the directory `learning/<slug>/` **and its workspace skeleton** — the subfolders `lessons/`, `assignments/`, `quizzes/`, `submissions/`, and `assets/` (e.g. `mkdir -p learning/<slug>/{lessons,assignments,quizzes,submissions,assets}`). Downstream skills write into these (`teach-lesson` → `lessons/` + `assets/`, `create-assignment` → `assignments/`, `teach-lesson`/`assess-understanding` → `quizzes/`, the Lyceum app → `submissions/`), so create them now rather than relying on each later write to materialize its parent.
...
- On a NEW subject: it CREATES `manifest.json` with `subject`, `slug`, `created`, `updated`, `scale`, `settings`, and the initial `current` block plus empty schema placeholders; and creates `learning/<slug>/` (with its `lessons/`, `assignments/`, `quizzes/`, `submissions/`, `assets/` subfolders) and `progress.md`.
